...
# from timm.models.layers import trunc_normal_, DropPat
#Directly replace with hardswish
...
class ResidualConv(nn.Module):
    def __init__(self, out_channels, isBN=True):
        super(ResidualConv, self).__init__()
        self.activateFunc = nn.GELU
        self.norm = nn.BatchNorm2d if isBN else LayerNorm
        ...
        self.acfun = self.activateFunc()
    def forward(self, x):
        block_x = self.double_conv(x)
        x = self.acfun(x + block_x)
        return  x

class SqueezeResidualConv(nn.Module):
    def __init__(self, in_channels, out_channels, isBN=True):
        super(SqueezeResidualConv, self).__init__()
        self.activateFunc = nn.GELU
        self.norm = nn.BatchNorm2d if isBN else LayerNorm
...
class LinearViewInteractiveAttention(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=2, sub_sample=False, bn_layer=True):
        """
        :param in_channels:
        :param inter_channels:
    ...
    def forward(self, x1, x2):
        ...
        theta_x11 = self.theta(x1).reshape(batch_size, self.inter_channels, -1)
        theta_x1 = theta_x11.permute(0, 2, 1)
        phi_x21 = self.phi(x2).reshape(batch_size, self.inter_channels, -1)
        phi_x2 = phi_x21.permute(0, 2, 1)

        # energy_space_1 = torch.matmul(theta_x1, phi_x2)
        ...
        # g_x11*energy_space_2s = C*H1W1 × S(H1W1)*H2W2 = (C*H2W2)' is rebuild C1*H1W1
        # y1 = torch.matmul( g_x11, energy_space_2s).contiguous()#C2*H2W2
        y1 = torch.matmul(torch.matmul( g_x11, theta_x1), phi_x21).contiguous()#C2*H2W2
        # g_x12*energy_space_1s = C*H2W2 × S(H2W2)*H1W1 = (C1*H1W1)' is rebuild C2*H2W2
        y2 = torch.matmul(torch.matmul( g_x21, phi_x2), theta_x11).contiguous()
        y1 = y1.reshape(batch_size, self.inter_channels, *x2.size()[2:])
        y2 = y2.reshape(batch_size, self.inter_channels, *x1.size()[2:])

        x1_ = self.double_conv(x1 + self.W(y2))
        x2_ = self.double_conv(x2 + self.W(y1))

        return self.acfun1(x1 + x1_), self.acfun2(x2 + self.W(y1))

class ViewInteractiveAttention(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=2, sub_sample=False, bn_layer=True):
        """
        :param in_channels:
        :param inter_channels:
...
class AdversialSupervised(nn.Module):
    def __init__(self, in_channels, out_channels=2, kernel_size=15, n_resblock=0):#kernel size 7目前最好
        super(AdversialSupervised, self).__init__()
        self.out_channels = out_channels
        self.squeezeRes = SqueezeResidualConv(in_channels, in_channels//2)
        self.nRes = [ResidualConv(in_channels//2) for i in range(n_resblock)]
        self.nRes = nn.Sequential(*self.nRes)
        self.conv1 = nn.Conv2d(in_channels//2, self.out_channels, kernel_size, padding=kernel_size//2, bias=False)
        # self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=1)
...
class AdversialSupervisedCosine(nn.Module):
    def __init__(self, in_channels, kernel_size=1):#7目前最好
        super(AdversialSupervisedCosine, self).__init__()
        # self.conv1 = nn.Conv2d(in_channels*2, 1, kernel_size, padding=kernel_size//2, bias=False)
        # self.sigmoid = nn.Sigmoid()
        self.cos = nn.CosineSimilarity(dim=2, eps=1e-6)

    def forward(self, x1, x2):
        batch_size, channel = x1.size()[0:2]
        # print(x1.shape)
        # print(x2.shape)
        x1 = x1.view(batch_size, channel, -1).permute(0, 2, 1)
        x2 = x2.view(batch_size, channel, -1).permute(0, 2, 1)
        # dist = (x1 - x2).pow(2).sum().sqrt()
        #衡量两个向量在每通道位置上的相似度
        dist = self.cos(x1, x2).mean(dim=1)
        # print(dist)
        # print(dist.shape)
        # x = F.softmax(x, dim=1)
        # x = x.view(batch_size, x.size(1), -1)
        # return torch.mean(x, dim=1)#index 
        return dist#index 