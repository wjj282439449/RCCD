...
@register_model
def getEncoder(pretrained=False, backbone_scale='tiny', in_22k=True, classes=1000, resolution=224, **kwargs):
    convnextParams = {
        "tiny": {
            "depths": [3, 3, 9, 3],
            "dims": [96, 192, 384, 768]
    ...
    if "resnet" in backbone_scale:
        model = globals()[backbone_scale](pretrained=pretrained, **kwargs)
        # model = resnet18(pretrained=pretrained, **kwargs)
    elif backbone_scale in ["tiny", "small", "base", "large", "xlarge"]:
        depths, dims = convnextParams[backbone_scale]["depths"], convnextParams[backbone_scale]["dims"]
        ...
        if pretrained: 
            ...
            if classes != 1000 and classes != 21841:   
                ...
                checkpoint['model']['head.bias'] = random_state_dict['head.bias']   
            model.load_state_dict(checkpoint["model"])    
    elif "convnextv2" in backbone_scale:
        model = timm.create_model(backbone_scale, features_only=True, pretrained=True)
    return model

# @register_model
# def getEncoder(pretrained=False, backbone_scale='tiny', in_22k=True, classes=1000, resolution=224, **kwargs):
#     model = timm.create_model(backbone_scale, features_only=True, pretrained=pretrained)
#     return model