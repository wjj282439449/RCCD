...
class Augmentation:
    ...
    def randomSpaceAugment(self,image, source_size=(256,256), angle=None, scale=(0.8, 1.25), unoverlap=None):
        ...
        if unoverlap==None:
            ...
            crop_w_start_img1 = img_center_x-(source_size[1]*(math.sin(angleRad)+math.cos(angleRad))*scale_times)//2
            crop_w_end_img1 = img_center_x+(source_size[1]*(math.sin(angleRad)+math.cos(angleRad))*scale_times)//2-source_size[1]
            crop_h_start_img1 = img_center_y-(source_size[0]*(math.sin(angleRad)+math.cos(angleRad))*scale_times)//2
            crop_h_end_img1 = img_center_y+(source_size[0]*(math.sin(angleRad)+math.cos(angleRad))*scale_times)//2-source_size[0]
    ...
    def randomSpaceAugmentWithGlobalMirrorCrop(self,image, source_size=(256,256), angle=None, scale=(0.8, 1.25), unoverlap=None):
        if angle == None:
            angle = transforms.RandomRotation.get_params([-180, 180]) 
        if isinstance(angle,list):
            angle = random.choice(angle)
        ...
        if unoverlap==None:
            ...
            img_center_y = img_h//2
            crop_w_start_img1 = 0
            crop_w_end_img1 = img_w-source_size[1]
            crop_h_start_img1 = 0
            crop_h_end_img1 = img_h-source_size[0]
            crop_w_range_img1 = (crop_w_start_img1, crop_w_end_img1)
            crop_h_range_img1 = (crop_h_start_img1, crop_h_end_img1)
            crop_w_start_x_img1 = int(torch.empty(1).uniform_(*crop_w_range_img1).item())
            crop_h_start_y_img1 = int(torch.empty(1).uniform_(*crop_h_range_img1).item())
            for each_img in range(len(image)):
                image[each_img] = image[each_img].crop((crop_w_start_x_img1, crop_h_start_y_img1, crop_w_start_x_img1+source_size[1], crop_h_start_y_img1+source_size[0]))
        else:
            img_h, img_w = np.array(image[0]).shape[0:2]            
    ...
    def randomSpaceAugmentWithRotateFlip(self,image, source_size=(256,256), angle=[0,90,180,270], scale=(0.8, 1.25), unoverlap=None):
        if isinstance(angle,list):
            angle = random.choice(angle)
        for i in range(len(image)):
            image[i] = image[i].rotate(angle)
